...
//--------------------------------------------BCD------------------------------------------
char Buff[]=	{   0x18, //sec     7th bit ch=0
								0x9, //min 		
								0x8, // hr 
								0x1, //week day
								0x4, //date
...
void RTC_CLOCK_DISPLAY()
{
	...
	LCD_WRITE_STRING(data[0]);

	LCD_WRITE_STRING(":");
	LCD_WRITE_STRING(data[2]);

	LCD_WRITE_STRING(":");
	LCD_WRITE_STRING(data[1]);

	LCD_WRITE_COMREG(0xC0); //SET CURSOR POSITION
	//LCD_WRITE_STRING(" d ");
	LCD_WRITE_STRING(data[4]);
	LCD_WRITE_STRING("/");

	LCD_WRITE_STRING(data[5]);
	LCD_WRITE_STRING("/");
	LCD_WRITE_STRING(data[6]);
